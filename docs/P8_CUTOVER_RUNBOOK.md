...
uv run pytest
uv run aelia config doctor
uv run aelia llm doctor --probe
uv run aelia db init
```
...
```text
AELIA_ADAPTER_MODE=shadow
```

Ingest a versioned envelope:

```bash
uv run aelia adapter ingest tests/fixtures/adapter/inbound_envelope.json
```
...
```text
AELIA_ADAPTER_MODE=test_canary
AELIA_ADAPTER_CANARY_CONVERSATION_IDS=["test-conversation-id"]
AELIA_ADAPTER_CONVERSATION_LIMIT_PER_HOUR=1
```
...
| After reservation, before transport | Run `uv run aelia adapter recover` |
...
1. Set `AELIA_ADAPTER_MODE=shadow`.
