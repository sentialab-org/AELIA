# Polyverse Agent V2 — Requirement-to-Evidence Matrix
...
| Independent runtime backend with N platform gates | Pass locally | ADR 0017; `polyverse-backend`; `/health`, `/ready`, `/runtime/status`, ingress/receipt HTTP contracts; Node gates use `RuntimeClient` and contain no process launcher; live synthetic Discord-envelope smoke |
...
| V2 quality gate runs in CI | Pass | `.github/workflows/v2-quality.yml` reproduces locked format, lint, strict type-check, and test gates with read-only permissions |
| V2 is the documented default development path | Pass | root `README.md`, `.env.example`, and `make v2-*` entrypoints; V1 entrypoints live only under `legacy/v1-rust/` |
