...
const EVENT_PREFIX = "discord-selfbot:";
...
function normalizeDiscordMessage(
  ...
  const eventId = `${EVENT_PREFIX}${rawMessageId}`;
  const isDm = !message.guildId;
  const mentionIds = Array.from(message.mentions?.users?.keys?.() || [], String);

  return {
    schema_version: "1.0.0",
    delivery_id: stableId("discord-selfbot-delivery", rawMessageId),
    idempotency_key: stableId("discord-selfbot-ingress", rawMessageId),
...
function rawDiscordMessageId(eventId) {
  if (typeof eventId !== "string" || !eventId.startsWith(EVENT_PREFIX)) {
    throw new Error("Unsupported Discord self-bot event id");
  }
