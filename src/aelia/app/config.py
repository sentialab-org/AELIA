...
from aelia.llm.config import LlmStructuredOutputMode, LlmThinkingMode
...
class GateSettings(BaseModel):
    ...
    state_path: Path = Path("data/gate-journal.jsonl")
...
class Settings(BaseSettings):
    ...
    database_path: Path = Path("data/aelia.db")
    persona_manifest_path: Path = Path("src/aelia/persona/source/manifest.json")
    agent_actor_id: str = Field(default="aelia", min_length=1, max_length=256)
...
_SECRET_ENV_NAMES = {"AELIA_LLM_API_KEY", "AELIA_RUNTIME_GATE_TOKEN"}


def _env_name(field_name: str) -> str:
    return "AELIA_" + field_name.upper()
...
def _toml_settings_values(raw: Mapping[str, Any]) -> dict[str, Any]:
    ...
    values: dict[str, Any] = {
        ...
        "database_path": storage.get("database_path", "data/aelia.db"),
        ...
        "agent_actor_id": agent.get("actor_id", "aelia"),
...
def load_settings(
    *,
    ...
    selected_config = config_path or Path(
        os.environ.get("AELIA_CONFIG_FILE", _PROJECT_ROOT / "config.toml")
    )
    ...
    llm_key = os.environ.get("AELIA_LLM_API_KEY", secret_values.get("AELIA_LLM_API_KEY"))
    ...
    runtime_token = os.environ.get(
        ...
        secret_values.get("AELIA_RUNTIME_GATE_TOKEN"),
    )
