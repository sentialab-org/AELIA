...
from polyverse.adapters.runtime import RuntimeAdapter
from polyverse.app.config import LlmProvider, Settings, load_settings
from polyverse.app.logging import configure_json_logging, get_logger
...
from polyverse.llm.config import OpenAICompatibleConfig
from polyverse.llm.openai_compatible import (
    OpenAICompatibleLanguageGenerator,
)
from polyverse.models.autonomy import AutonomyPolicyConfig
from polyverse.persona.loader import PersonaIntegrityError, PersonaSourceLoader
from polyverse.runtime.observability import TraceObservabilityService
from polyverse.runtime.orchestrator import AutonomyOrchestrator
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="polyverse")
...
async def _run(args: argparse.Namespace) -> int:
    ...
    if args.command == "adapter" and args.adapter_command in {"ingest", "recover"}:
        ...
        if test_content is not None and settings.adapter_mode.value != "test_canary":
            raise ValueError("--test-content requires POLYVERSE_ADAPTER_MODE=test_canary")
        orchestrator = AutonomyOrchestrator(
            repository,
            agent_actor_id=settings.agent_actor_id,
            persona_specification=specification,
